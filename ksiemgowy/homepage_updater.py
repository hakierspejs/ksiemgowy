...
def serialize(dict_to_serialize):
...
def get_remote_state_dues(remote_state_path):
...
def update_git_remote_state(filepath, new_state, env):
...
def maybe_update_dues(database, git_env):
...
def maybe_update(database, deploy_key_path):
